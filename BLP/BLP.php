    ...
    isset($_POST['password']))
{
    ...
    for ($j = 0 ; $j < $usersRows ; ++$j)
    {
        ...
        if($userLevel == 1)
        {
            ...
            $queryMessages = "SELECT * FROM bell WHERE level='1'";
            ...
            $query = "SELECT * FROM bell WHERE level BETWEEN 1 AND 3";
...
_END;
        ...
        {
            echo "<div align=\"left\"><h2>You can read next messages:</h2>";
            $queryMessages = "SELECT * FROM bell WHERE level BETWEEN 1 AND 2 ";
            $messages = $connection->query($queryMessages);
            $messagesRows = $messages->num_rows;
            for ($i = 0 ; $i < $messagesRows ; ++$i)
            {
            ...
            echo "<div align=\"left\"><h2>You can write into next messages:</h2>";
            $query = "SELECT * FROM bell WHERE level BETWEEN 2 AND 3";
            $result = $connection->query($query);
            $resultRows = $result->num_rows;
            for ($a = 0 ; $a < $resultRows ; ++$a)
            {
...
_END;
        ...
        {
            echo "<div align=\"left\"><h2>You can read next messages:</h2>";
            $queryMessages = "SELECT * FROM bell WHERE level BETWEEN 1 AND 3 ";
            $messages = $connection->query($queryMessages);
            $messagesRows = $messages->num_rows;
            for ($i = 0 ; $i < $messagesRows ; ++$i)
            {
            ...
            echo "<div align=\"left\"><h2>You can write into next messages:</h2>";
            $query = "SELECT * FROM bell WHERE level = 3";
            $result = $connection->query($query);
            $resultRows = $result->num_rows;
            for ($a = 0 ; $a < $resultRows ; ++$a)
            {
